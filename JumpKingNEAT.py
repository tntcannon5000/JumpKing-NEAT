...
class JKGame:
	...
	def __init__(self, n_kings, n_levels):
		...
		self.kings = []
		# Initialize n number of kings for a generation
...
# Calculate if the platform is left or right of the king, the relative y position of the king and the distance to the platform
def calculate_distances(env, king, platform):
    
    #Ignore the side walls as we do not need them
    # We will take into account platfroms that are 154 units above the king
	if (platform.y < king.y) and (platform.width > 8) and abs(platform.y-king.y) < 154:
		deltax = platform.x - king.x
		# If the platform is to the left of the king
		if deltax <= 0:
			l = 1
			r = 0 
		...
		# If the platform is to the right of the king
		elif deltax > 0:
			l = 0
			r = 1	
...
def get_surrounding_platforms(env, king):
	...
	# Pad out the surrounding_platforms list as we have there can be a maximum of 6 platforms 154 units above the king
